...
describe('invalidateChickenCache', () => {
  it('clears the chickens cache', async () => {
    redisMock.keys.mockResolvedValue([]);

    await invalidateChickenCache();

    expect(redisMock.keys).toHaveBeenCalledWith(`${CACHE_KEYS.CHICKENS}:*`);
  });
});
