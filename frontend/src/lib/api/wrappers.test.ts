...
import { getChickenBreeds, getAvailableChickens, getChickenBySlug } from './chickens';
...
describe('chickens api', () => {
  it('getChickenBreeds requests /chickens', async () => {
    const fetchMock = mockFetchJson([]);
    await getChickenBreeds();
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:3001/api/chickens', expect.anything());
  });

  it('getAvailableChickens requests /chickens/available', async () => {
    const fetchMock = mockFetchJson([]);
    await getAvailableChickens();
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:3001/api/chickens/available', expect.anything());
  });

  it('getChickenBySlug requests /chickens/:slug', async () => {
    const fetchMock = mockFetchJson({});
    await getChickenBySlug('sussex');
    expect(fetchMock).toHaveBeenCalledWith('http://localhost:3001/api/chickens/sussex', expect.anything());
  });
});
